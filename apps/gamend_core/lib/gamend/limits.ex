defmodule Gamend.Limits do
  ...
  `config :gamend_core, Gamend.Limits, key: value` or at runtime
  via `Application.put_env(:gamend_core, Gamend.Limits, [...])`.
  ...
  ## Usage in schemas
      ...
      import Gamend.Limits, only: [get: 1, validate_metadata_size: 2]

      changeset
      |> validate_length(:title, max: Gamend.Limits.get(:max_group_title))
  ...
  """
  @spec all() :: map()
  def all, do: Map.new(__settings__(), &{&1.key, Gamend.Settings.get(__MODULE__, &1.key)})

  @doc """
  Returns the current value for the given limit key.

  Reads from `Application.get_env(:gamend_core, Gamend.Limits)` first,
  falling back to the compiled default.
  """
  @spec get(atom()) :: integer() | any()
  def get(key) when is_atom(key), do: Gamend.Settings.get(__MODULE__, key)
