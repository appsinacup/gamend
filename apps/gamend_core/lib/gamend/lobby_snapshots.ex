defmodule Gamend.LobbySnapshots do
  @moduledoc """
  ...
  alias Gamend.LobbySnapshots.{Blob, Event, Snapshot, Writer}
  alias Gamend.Repo
  ...
  *current* lobby and captures against that, skipping entirely when the caller
  ...
  defp caller_lobby_id(caller) do
    case caller_id(caller) do
      nil -> nil
      id -> with %{lobby_id: lobby_id} <- Gamend.Accounts.get_user(id), do: lobby_id
    end
  end
      ...
      keys ->
        ...
        Enum.flat_map(members, fn %{id: user_id} ->
          ...
          |> Enum.flat_map(fn key ->
            Gamend.KV.list_entries(user_id: user_id, key: key, page_size: page_size)
          end)
  ...
  # window (see `Gamend.Retention`) is what bounds it.
