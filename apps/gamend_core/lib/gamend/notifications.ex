defmodule Gamend.Notifications do
  @moduledoc """
  ...
  def subscribe(user_id) when is_binary(user_id) do
    Phoenix.PubSub.subscribe(Gamend.PubSub, "notifications:user:#{user_id}")
  end
  ...
  def unsubscribe(user_id) when is_binary(user_id) do
    Phoenix.PubSub.unsubscribe(Gamend.PubSub, "notifications:user:#{user_id}")
  end
  ...
  defp notifications_version(user_id) do
    Gamend.Cache.get!({:notifications, :version, user_id}) || 1
  end
  ...
  defp notification_row_version, do: Gamend.Cache.get!({:notifications, :row_version}) || 1
  defp bump_notification_row, do: Gamend.Cache.bump_version({:notifications, :row_version})

  @doc false
  @spec invalidate_notifications_cache(user_id()) :: :ok
  def invalidate_notifications_cache(user_id) when is_binary(user_id) do
    Gamend.Async.run(fn ->
      _ = Gamend.Cache.bump_version({:notifications, :version, user_id})
  ...
  defp maybe_filter_recipient(query, filters) do
    ...
    case Gamend.UUIDv7.cast_or_nil(recipient_id) do
      nil -> query
  ...
  defp maybe_filter_sender(query, filters) do
    case Gamend.UUIDv7.cast_or_nil(Map.get(filters, "sender_id")) do
      nil -> query
  ...
  def send_notification(sender_id, attrs) when is_binary(sender_id) and is_map(attrs) do
    ...
    cond do
      ...
      true ->
        max = Gamend.Limits.get(:max_notifications_per_user)
