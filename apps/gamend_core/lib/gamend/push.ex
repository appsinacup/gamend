defmodule Gamend.Push do
  ...
  alias Gamend.Push.FanoutWorker
  ...
  defp push_version(user_id) do
    Gamend.Cache.get!({:push, :version, user_id}) || 1
  end
  ...
  def invalidate_push_cache(user_id) when is_binary(user_id) do
    Gamend.Async.run(fn ->
      _ = Gamend.Cache.bump_version({:push, :version, user_id})
      :ok
    end)
  ...
  def register_token(user_id, attrs) when is_binary(user_id) and is_map(attrs) do
    ...
    result =
      Gamend.Lock.serialize(:push_tokens, user_id, fn ->
  ...
  @default_providers %{
    "fcm" => Gamend.Push.Providers.FCM,
    "apns" => Gamend.Push.Providers.APNs
  }
  ...
  def force_log?, do: Gamend.Settings.get(__MODULE__, :adapter) == :log
  ...
  defp resolve_messages(user_ids, message_map) do
    Enum.reduce(user_ids, %{}, fn user_id, acc ->
      with {:ok, returned} <-
             Gamend.Hooks.internal_call(:before_push_send, [user_id, message_map]),
