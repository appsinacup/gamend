defmodule Gamend.Tournaments do
  @moduledoc """
  ...
  alias Gamend.Tournaments.Match
  ...
  defp dispatch_ready(tournament, match) do
    ...
    defer(fn ->
      Gamend.Async.run(fn ->
        Gamend.Hooks.internal_call(:tournament_match_ready, [payload])
      end)
  ...
  def resolve_match(match_id, winner) when is_binary(match_id) do
    with %Match{} = match <- Repo.get(Match, match_id) || {:error, :not_found},
         ...
         {:ok, _} <-
           Gamend.Hooks.internal_call(:before_tournament_result, [
  ...
  defp internal_resolve(tournament, match, winner, extra_metadata) do
    Gamend.Lock.serialize(:tournament_match, match.id, fn ->
  ...
  defp do_resolve(tournament, match, winner, extra_metadata) do
    ...
    defer(fn ->
      unless extra_metadata["bye"] do
        ...
        Gamend.Async.run(fn ->
          Gamend.Hooks.internal_call(:after_tournament_match_resolved, [payload])
  ...
  def update_match_metadata(match_id, map) when is_binary(match_id) and is_map(map) do
    Gamend.Lock.serialize(:tournament_match, match_id, fn ->
  ...
  defp mark_expired(tournament, match, now) do
    case match |> Match.changeset(%{expired_at: now}) |> Repo.update() do
      {:ok, match} ->
        ...
        defer(fn ->
          Gamend.Async.run(fn ->
            Gamend.Hooks.internal_call(:tournament_match_expired, [payload])
          end)
        end)
  ...
  defp do_finish_side_effects(tournament) do
    defer(fn ->
      ...
      Gamend.Async.run(fn ->
        Gamend.Hooks.internal_call(:after_tournament_finished, [tournament, standings])
      end)
  ...
  defp report_winner_quest_event(%Match{} = match) do
    ...
    if winner do
      Gamend.Quests.report_event(winner.leader_id, "match_won", 1, %{
        "tournament_id" => match.tournament_id
      })
    end
