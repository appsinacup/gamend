defmodule Gamend.AccountsTest do
  ...
  describe "delete_user/1 and lobby/token cleanup" do
    test "deleting a host with other members transfers host and deletes tokens" do
      ...
      {:ok, lobby} =
        Gamend.Lobbies.create_lobby(%{title: "hosted", host_id: host.id})

      # add member to lobby
      {:ok, _} = Gamend.Lobbies.create_membership(%{lobby_id: lobby.id, user_id: member.id})
      ...
      updated = Repo.get!(Gamend.Lobbies.Lobby, lobby.id)
    ...
    test "deleting last member host deletes lobby and tokens" do
      ...
      {:ok, lobby} =
        Gamend.Lobbies.create_lobby(%{title: "single", host_id: host.id})
      ...
      # host and lobby gone
      refute Repo.get(Accounts.User, host.id)
      refute Repo.get(Gamend.Lobbies.Lobby, lobby.id)
  ...
  describe "list_all_users/2 (admin search) and its boundary with search_users/2" do
    ...
    test "facets filter (online) and count agree with the listing" do
      online = user_fixture()
      {:ok, _} = online |> Ecto.Changeset.change(is_online: true) |> Gamend.Repo.update()
