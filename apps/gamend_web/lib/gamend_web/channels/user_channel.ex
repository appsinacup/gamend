defmodule GamendWeb.UserChannel do
  @moduledoc """
      ...
      {:ok, user_id} ->
        case current_scope do
          %Scope{user_id: ^user_id} ->
            ...
            GamendWeb.ConnectionTracker.register_user_channel(user_id)
  ...
  def handle_in("webrtc:offer", %{"sdp" => _} = offer_json, socket) do
    with :ok <- check_ws_rate_limit(socket),
      ...
      {:ok, peer} =
        GamendWeb.WebRTCPeer.start_link(
          user_id: socket.assigns.current_scope.user_id,
          channel_pid: self()
        )

      GamendWeb.WebRTCPeer.handle_offer(peer, offer_json)
  ...
  def handle_in("webrtc:ice", %{"candidate" => _} = candidate_json, socket) do
    with :ok <- check_ws_rate_limit(socket),
      ...
      case Map.get(socket.assigns, :webrtc_peer) do
        ...
        peer ->
          GamendWeb.WebRTCPeer.add_ice_candidate(peer, candidate_json)
          {:reply, {:ok, %{}}, socket}
      end
    end
  end
  ...
  def handle_in("webrtc:close", _payload, socket) do
    with :ok <- check_ws_rate_limit(socket) do
      case Map.get(socket.assigns, :webrtc_peer) do
        ...
        peer ->
          if Process.alive?(peer), do: GamendWeb.WebRTCPeer.close(peer)
  ...
  def handle_info({:after_join, %User{} = user}, socket) do
    ...
    # projection it writes (see Gamend.Presence). Tracking here rather than
    ...
    Phoenix.PubSub.subscribe(Gamend.PubSub, "tournaments:user:#{user.id}")
    Phoenix.PubSub.subscribe(Gamend.PubSub, "matchmaking:user:#{user.id}")

    # Live wallet balance + inventory updates
    Gamend.Economy.subscribe(user.id)
    Gamend.Inventory.subscribe(user.id)
  ...
  @impl true
  # Game-defined events from Gamend.Realtime.push_to_user/3. The name is
  ...
  def terminate(_reason, socket) do
    ...
    if user_id do
      ...
      Phoenix.PubSub.unsubscribe(Gamend.PubSub, "tournaments:user:#{user_id}")
      Phoenix.PubSub.unsubscribe(Gamend.PubSub, "matchmaking:user:#{user_id}")

      # Cancel matchmaking tickets
      Gamend.Matchmaking.cancel(user_id)
  ...
  defp parse_optional_id(value), do: Gamend.UUIDv7.cast_or_nil(value)
  ...
  defp check_ws_rate_limit(socket) do
    if Gamend.Settings.get(GamendWeb.Plugs.RateLimiter, :enabled) do
      user_id = socket.assigns.current_scope.user_id
      limit = Gamend.Settings.get(GamendWeb.Plugs.RateLimiter, :ws_limit)
      window = Gamend.Settings.get(GamendWeb.Plugs.RateLimiter, :ws_window_ms)

      case GamendWeb.RateLimit.hit("ws:#{user_id}", window, limit) do
        {:allow, _count} ->
          :ok
  ...
  defp check_ice_rate_limit(socket) do
    if Gamend.Settings.get(GamendWeb.Plugs.RateLimiter, :enabled) do
      user_id = socket.assigns.current_scope.user_id
      limit = Gamend.Settings.get(GamendWeb.Plugs.RateLimiter, :ice_limit)
      window = Gamend.Settings.get(GamendWeb.Plugs.RateLimiter, :ice_window_ms)

      case GamendWeb.RateLimit.hit("ice:#{user_id}", window, limit) do
        {:allow, _count} ->
          :ok
