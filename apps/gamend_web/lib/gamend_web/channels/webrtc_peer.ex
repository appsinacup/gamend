defmodule GamendWeb.WebRTCPeer do
  @moduledoc """
  ...
  def init(opts) do
    ...
    GamendWeb.ConnectionTracker.register(:webrtc_peer, %{user_id: user_id})
  ...
  defp dc_rate_limit_allowed?(user_id) do
    enabled? = Gamend.Settings.get(GamendWeb.Plugs.RateLimiter, :enabled)

    if enabled? do
      limit = Gamend.Settings.get(GamendWeb.Plugs.RateLimiter, :dc_limit)
      window = Gamend.Settings.get(GamendWeb.Plugs.RateLimiter, :dc_window_ms)

      case GamendWeb.RateLimit.hit("dc:#{user_id}", window, limit) do
        {:allow, _count} -> true
