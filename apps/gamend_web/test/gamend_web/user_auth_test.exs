defmodule GamendWeb.UserAuthTest do
  ...
  describe "log_in_user/3" do
    ...
    test "triggers hooks.after_user_logged_in for non-token logins", %{conn: conn, user: user} do
      Application.put_env(:gamend_core, :hooks_module, TestHooksLogin)
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      GamendWeb.endpoint().subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        GamendWeb.endpoint().subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
