defmodule Gamend.Hooks do
  ...
  from the Gamend and run custom game logic.

  ## Setup

  1. Create a module implementing this behaviour
  2. Configure it in your Gamend instance
  ...
  @typedoc "A user struct from Gamend.Accounts.User"
  @type user :: Gamend.Accounts.User.t()

  @typedoc "A lobby struct from Gamend.Lobbies.Lobby"
  @type lobby :: Gamend.Lobbies.Lobby.t()

  @typedoc "A group struct from Gamend.Groups.Group"
  @type group :: Gamend.Groups.Group.t()

  @typedoc "A party struct from Gamend.Parties.Party"
  @type party :: Gamend.Parties.Party.t()
  ...
  # Match payloads are `Gamend.Tournaments.Match` structs with
  ...
  # match at its deadline via `Gamend.Tournaments.resolve_match/2`.
  @doc """
  ...
  When Gamend executes a hook function, it may inject a `:caller` into the
  ...
  def caller do
    case Application.get_env(:gamend_sdk, :stub_mode, :raise) do
      ...
      _ ->
        raise "#{__MODULE__}.caller/0 is a stub - only available at runtime on Gamend"
    end
  end
  ...
  If the caller is a `Gamend.Accounts.User` struct, returns its `id`.
  ...
  def caller_id do
    case Application.get_env(:gamend_sdk, :stub_mode, :raise) do
      :placeholder -> 0
      _ -> raise "#{__MODULE__}.caller_id/0 is a stub - only available at runtime on Gamend"
    end
  end
  ...
  the caller is already a `%Gamend.Accounts.User{}`.

  In the full Gamend application this may also resolve ids/maps via the DB.
  ...
  def caller_user do
    case Application.get_env(:gamend_sdk, :stub_mode, :raise) do
      ...
      _ ->
        raise "#{__MODULE__}.caller_user/0 is a stub - only available at runtime on Gamend"
    end
  end
end
